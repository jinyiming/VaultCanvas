...
fn build_crypto_output(directory: &str, input: &str, mode: CryptoMode) -> String {
    ...
    match mode {
        CryptoMode::Encrypt => format!("{directory}\\{name}.enc"),
        CryptoMode::Decrypt => {
            if name.to_lowercase().ends_with(".enc") {
                format!("{directory}\\{}", &name[..name.len() - 4])
            } else {
                format!("{directory}\\{name}.dec")
            }
        }
    }
}
...
fn build_stego_output(directory: &str, input: &str, mode: StegoModeUi) -> String {
    let name = file_name(input);
    match mode {
        StegoModeUi::Embed => format!("{directory}\\{name}.ste"),
        StegoModeUi::Extract => {
            if name.to_lowercase().ends_with(".ste") {
                format!("{directory}\\{}", &name[..name.len() - 4])
            } else {
                format!("{directory}\\{name}.out")
            }
        }
    }
}
